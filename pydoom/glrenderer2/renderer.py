"""GL frame render v2 (new-hw-accel, P0 state-cache slice).

Same pixels as v1's FrameRenderer (method-for-method mirror: same
constructor, same public calls, so the viewer swaps one line), driven
through the caching GL facade (gl.py) with a frame profiler
(profile.py): same programs, same draw order, fewer GL calls.

Programs, LUTs and geometry layouts are shared with v1 (shaders,
camera and sky math import straight from pydoom.glrender); only the
GL call sites change. V1 stays frozen as the parity reference.
"""
...
class FrameRenderer2:
    ...
    def __init__(self, res, w: int, h: int, linear: bool = False) -> None:
        ...
        self._sky_ibo = int(GL.glGenBuffers(1))
        GL.glBindBuffer(GL.GL_ELEMENT_ARRAY_BUFFER, self._sky_ibo)
    ...
    def render(self, viewx: int, viewy: int, viewz: int,
        ...
        # NOTE: P0.3 fence: last frame's dynamic draws must be done
        # before refilling sprite/sky/psprite VBOs. Poll-ish wait
        # (1 ms): expiry just counts a stall, the per-frame orphan on
        # upload keeps correctness either way.
        if self._fence is not None:
            if not GL.wait_fence(self._fence):
                self.prof.count("fence_stall")
            GL.delete_fence(self._fence)
            self._fence = None
        GL.glBindFramebuffer(GL.GL_FRAMEBUFFER, self._fbo)
        ...
        with self.prof.scope("walls"):
            ...
            GL.glUniformMatrix4fv(self._loc(self.wall_double_prog,
                                            "uViewProj"), 1, True, vp)
            GL.glUniform2f(self._loc(self.wall_double_prog,
                                     "uViewPos"),
                           viewx / 65536.0, viewy / 65536.0)
            GL.glUniform2f(self._loc(self.wall_double_prog,
                                     "uViewDir"),
        ...
        with self.prof.scope("planes"):
            ...
            GL.glUniformMatrix4fv(self._loc(self.plane_prog,
                                            "uViewProj"),
                                  1, True, vp)
        ...
        # (overlay/text/automap VBOs upload outside render and stay on
        # orphan-only, like v1: tiny, unmeasured).
        if self._fence is not None:
            GL.delete_fence(self._fence)
        self._fence = GL.place_fence()
        self.prof.frame()  # NOTE: one render call is one frame
        err = GL.glGetError()
    ...
    def draw_sprites(self, res, billboards, vp,
        ...
        with self.prof.scope("sprites"):
            GL.glUseProgram(self.sprite_prog)
            self._pal(self.sprite_prog, pal_index)
            GL.glUniformMatrix4fv(self._loc(self.sprite_prog,
                                            "uViewProj"),
                                  1, True, vp)
            GL.glActiveTexture(GL.GL_TEXTURE2)
            GL.glBindTexture(GL.GL_TEXTURE_2D, res.colormap_tex)
    ...
    def draw_psprite(self, tex_id: int, w: int, h: int,
        ...
        import numpy as np
        with self.prof.scope("psprite"):
            sx, sy = self._w / 320.0, self._h / 200.0
            x0, y0 = ((1 + bobx - leftoff) * sx,
                      (32 + boby - topoff) * sy)
            x1, y1 = x0 + w * sx, y0 + h * sy
            verts = np.array([
                x0 / (self._w / 2) - 1.0, 1.0 - y0 / (self._h / 2),
                0.0, 0.0,
                x0 / (self._w / 2) - 1.0, 1.0 - y1 / (self._h / 2),
                0.0, float(h),
    ...
    def draw_text_quad(self, tex_id: int, x: int, y: int, w: int,
        ...
        import numpy as np
        with self.prof.scope("text"):
            verts = np.array([
                x / (self._w / 2) - 1.0, 1.0 - y / (self._h / 2),
                0.0, 0.0,
